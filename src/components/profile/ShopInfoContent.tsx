...
function StatusBadge({ status }: { status?: string }) {
  ...
  return (
    <Badge className={styles[status] || 'bg-muted text-muted-foreground border-border'}>
      {status}
    </Badge>
  );
}
...
function FlagItem({ label, value }: { label: string; value: boolean | null | undefined }) {
  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-2 rounded-md hover:bg-muted transition-colors duration-200">
    ...
    <div className={`flex items-start gap-4 ${isPage ? 'mb-6' : 'px-6 pt-6 pb-3'}`}>
      {shopLogo ? (
        <img
          ...
          className={`${isPage ? 'w-16 h-16' : 'w-10 h-10'} rounded-lg object-cover border border-border`}
        />
      ) : (
        <div className={`${isPage ? 'w-16 h-16' : 'w-10 h-10'} rounded-lg bg-muted flex items-center justify-center`}>
          <Store className={`${isPage ? 'w-7 h-7' : 'w-5 h-5'} text-muted-foreground`} />
        </div>
      )}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-3 flex-wrap">
          <h2 className={`${isPage ? 'text-xl' : 'text-base'} font-semibold truncate text-foreground`}>
            {shopName || `Shop ${shopId}`}
          </h2>
          <StatusBadge status={info?.status} />
        </div>
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          <span className="text-sm text-muted-foreground font-mono">ID: {shopId}</span>
          {info?.region && (
        ...
        <div>
          ...
          <div className="space-y-1">
            {info.sip_affi_shops.map((shop: SipAffiShop, i: number) => (
              <div key={i} className="flex items-center gap-2 px-2 py-1 text-sm bg-muted rounded">
