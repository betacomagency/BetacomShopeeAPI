...
const SEVERITY_CONFIG: Record<Severity, { label: string; className: string; icon: typeof AlertTriangle }> = {
  INFO: { label: 'Thông tin', className: 'bg-info/10 text-info', icon: History },
  WARNING: { label: 'Cảnh báo', className: 'bg-warning/10 text-warning', icon: AlertTriangle },
  HIGH: { label: 'Nghiêm trọng', className: 'bg-destructive/10 text-destructive', icon: AlertTriangle },
  CRITICAL: { label: 'Cực kỳ nghiêm trọng', className: 'bg-destructive/20 text-destructive', icon: Ban },
};

// Change type config
const CHANGE_TYPE_CONFIG: Record<ChangeType, { label: string; icon: typeof Tag; className: string }> = {
  price_change: { label: 'Sửa giá', icon: Tag, className: 'text-brand' },
  stock_change: { label: 'Sửa kho', icon: Box, className: 'text-info' },
  status_change: { label: 'Trạng thái', icon: FileText, className: 'text-purple-600' },
  content_change: { label: 'Nội dung', icon: FileText, className: 'text-muted-foreground' },
  violation: { label: 'Vi phạm', icon: Ban, className: 'text-destructive' },
  product_created: { label: 'Mới tạo', icon: Plus, className: 'text-success' },
  product_deleted: { label: 'Đã xóa', icon: Trash2, className: 'text-destructive' },
  ...
  const renderLogItem = (log: HistoryLog) => {
    ...
    return (
      <div
        key={log.id}
        className={cn(
          'border-b last:border-b-0 p-4 hover:bg-accent transition-colors',
          !log.is_read && 'bg-info/10 border-l-4 border-l-info'
        )}
      >
        <div className="flex items-start gap-3">
          {/* Icon */}
          <div className={cn('p-2 rounded-lg bg-muted flex-shrink-0', changeConfig.className)}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <span className="text-xs text-muted-foreground" title={formatDateTime(log.detected_at)}>
            ...
            {log.change_type === 'violation' && (
              <div className="mt-2 p-2 bg-destructive/10 rounded-lg text-sm">
                <p className="text-destructive font-medium">{log.violation_type}</p>
                {log.violation_reason && (
                  <p className="text-destructive text-xs mt-1">{log.violation_reason}</p>
                )}
                {log.violation_suggestion && (
                  <p className="text-foreground text-xs mt-1 italic">
            ...
            {log.is_anomaly && log.anomaly_reason && (
              <div className="mt-2 p-2 bg-brand/10 rounded-lg text-sm text-brand">
    ...
    <Card className="border-0 shadow-sm flex flex-col h-[calc(100vh-73px)]">
      <CardContent className="p-0 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-shrink-0">
          {/* Stats bar */}
          {stats && (
            <div className="px-4 py-2 bg-gradient-to-r from-muted to-card border-b flex items-center gap-4 overflow-x-auto">
              <div className="flex items-center gap-1.5 text-xs">
                <Bell className="w-3.5 h-3.5 text-info" />
                <span className="text-muted-foreground">Chưa đọc:</span>
                <span className="font-semibold text-info">{stats.unread_count}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs">
                <AlertTriangle className="w-3.5 h-3.5 text-warning" />
                <span className="text-muted-foreground">Cảnh báo:</span>
                <span className="font-semibold text-warning">{stats.warning_count}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs">
                <Ban className="w-3.5 h-3.5 text-destructive" />
                <span className="text-muted-foreground">Vi phạm:</span>
                <span className="font-semibold text-destructive">{stats.violations}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs">
                <Tag className="w-3.5 h-3.5 text-brand" />
                <span className="text-muted-foreground">Sửa giá:</span>
                <span className="font-semibold text-brand">{stats.price_changes}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs">
                <Box className="w-3.5 h-3.5 text-info" />
                <span className="text-muted-foreground">Sửa kho:</span>
                <span className="font-semibold text-info">{stats.stock_changes}</span>
              </div>
              <div className="flex items-center gap-1.5 text-xs">
                <History className="w-3.5 h-3.5 text-muted-foreground" />
                <span className="text-muted-foreground">Hôm nay:</span>
                <span className="font-semibold text-foreground">{stats.today_count}</span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between border-b bg-white px-2 gap-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0 py-2">
              ...
              <Button
                ...
                className={cn('h-8 text-xs', showUnreadOnly && 'bg-info hover:bg-info/90')}
              >
